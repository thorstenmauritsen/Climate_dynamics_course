...
A climatic catastrophe is sometimes referred to as a forced transition of climate system to a new climate state at a rate which is more rapid than the rate of change of the external forcing ({\em wikipedia}). In this case we may be thinking of some aspect of the climate that is being changed through external forcing at an accelerating rate or all of a sudden over a short period of time. One can also imagine that something which didn't happen before starts to happen. In order to qualify as a catastrophe we usually also think of it having a large negative impact on something, say society or nature, but here that is not so central.
...
But also a strong increase in extreme events of some kind would usually be considered catastrophic. 
...
Let us consider natural variability in the mixed-layer model:
...
The variance of an AR(1) process is determined by this memory parameter and the driving white noise process. For the mixed layer model, this driving white noise is in the radiative forcing, $\sigma_F$, in units of Wm$^{-2}$, which we must first convert to the equivalent standard deviation in temperature by accounting for the size of the timestep and the depth of the mixed layer:
...
A strong memory in the system implies $\alpha$ is close to unity, and that the variability is large ($\alpha$=1  is a pure red noise process, whose variability grows with each timestep to infinity).


Certain aspects of climate change depend mainly on the mean state, for instance thermosteric sea level rise scales foremost with the global mean temperature. However, in many cases it is mainly during weather extremes that we spur the changing climate; cold-spells and heat waves imprint themselves in our memory and cause damages. Many of these extremes are associated with absolute thresholds. For instance, most mammals die from heat stress if the wet bulb temperature exceeds around 35$^\circ$C, and extreme precipitation only becomes a severe problem when the water can no longer run off in natural watersheds and existing sewers. The former is a case that hardly ever occurs in the present-day climate, but may become frequent in the future.

Much of this can be understood from simple linear shifts in variability around a mean state. In this case the processes that cause the variability do not depend on the mean state of the system. 


Variability in a linear world, shift of pdf, likelyhood of extremes increase dramatically


\begin{figure}
	\begin{center}
		\includegraphics[width=15 cm]{../Plots/variability_shift_distribution.pdf}
	\end{center}
	\caption{ Caption. } 
...
\begin{figure}
	\begin{center}
		\includegraphics[width=15 cm]{../Plots/Stability_theory_parabolic}
	\end{center}
	\caption{ Caption. } 
...
We saw how non-linear changes in extremes can occur, even when there is no tipping point. So, how will we ever be able to tell if we are approaching a tipping point?  Some claim that we will see the approach to a tipping point, because the system behaviour will begin to change. Let's build a system with a tipping point by taking an example we have already studied, temperature-dependent climate feedbacks, which we can approximate with a quadratic equation,

\begin{equation}
N=F+\lambda_0 T + b T^2.
\end{equation}

Here, $\lambda_0$ is the feedback at $T=0$ and $b$ determines how the feedback changes with warming. If we choose $b=\frac{1}{2}$,  $\lambda_0 = \frac{3}{2}$, and $F=0$, we have a system in equilibrium ($N=0$) at $T=0$. But if we perturb the system with $F_C = \frac{9}{8}$ Wm$^{-2}$, we raise the imbalance curve so that it just touches the zero-intercept at $T=1.5$ K, which is an unstable equilibrium. Any positive perturbation in temperature above $1.5$ K (or additional forcing) will lead to runaway warming, as shown in Figure \ref{fig:parabolic_theory}. We can also see this reflected in the potential curves that we encountered in Chapter \ref{chapter:state-dependent}. The case with zero forcing still contains a local minimum, where our system can safely remain, but perturbing the system with the critical forcing $F_C$ leads to the limiting case, where there is no longer a local minimum in the potential.

\begin{figure}
	\begin{center}
		\includegraphics[width=15 cm]{../plots/Stability_mlm}
	\end{center}
	\caption{ Caption } 
...
We can see what happens when we approach this tipping by slowly inching the mixed-layer model towards $F_C$. The orange curve in Figure \ref{fig:bifurc_parabolic} shows this slow increase in forcing, starting at an equilibrium solution of $F=1,T_{eq}=1$ and moving over 5000 monthly timesteps to just above $F_C$, where the forcing is held constant. After the mixed layer has responded to the additional $\frac{1}{8}$ Wm$^{-2}$ forcing, which still takes some time, it breaches the critical temperature $T_{C}=1.5$ and runs away to infinity.
...
For the quadratic model, $\alpha$ is dependent on temperature. Our $\lambda$ can no longer be determined by $\lambda_0$, but must be more generally described as the change in the energy balance with warming:
\begin{equation}
\lambda(T) = \frac{dN}{dT} = \lambda_0 + 2bT
\end{equation}

As the system temperature increases, the second-order term becomes increasingly important. For our parameter choice of $\lambda_0 < 0$ and $b > 0$, $\lambda$ becomes less negative with temperature, increasing the value of $\alpha$ towards unity.  

\begin{figure}
	\begin{center}
		\includegraphics[width=15 cm]{../plots/Stability_vari_1}
	\end{center}
	\caption{ Caption } 
...
\begin{figure}
	\begin{center}
		\includegraphics[width=15 cm]{../plots/Stability_qubicon}
	\end{center}
	\caption{ Caption } 
...
\begin{figure}
	\begin{center}
		\includegraphics[width=15 cm]{../plots/Stability_theory}
	\end{center}
	\caption{ Caption } 
