...
\section{Noise}
Not everyone has a large ensemble. And for purposes of efficiency or conceptual understanding, we may wish to represent variability in a simpler way than with a coupled climate model. We can represent internal variability, for example, as stochastic noise in the form of an error in the forcing term of the simple models we have already encountered. In the mixed layer model this becomes:

\begin{equation}
	C\frac{dT}{dt} = F + \lambda T + \epsilon,
	\label{eq:mlo-var}
\end{equation}

There are multiple types of noise. \textit{White noise} represents a Gaussian distribution in the error term, $\epsilon \sim N(\mu,\sigma^{2}) $. It is completely random and has equal power density at all timescales. We could use white noise to represent changes in the forcing. But the mixed layer will save the history of this white noise as heat content... red noise


Conceptualise IV as noise
